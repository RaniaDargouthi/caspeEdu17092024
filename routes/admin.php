...
Route::group(['prefix' => $prefix, 'namespace' => 'Admin', 'middleware' => ['web', 'admin_locale']], function () use ($prefix) {
    ...
    Route::group(['middleware' => 'admin'], function () {
        ...
        Route::group(['prefix' => 'ebooks'], function () {
            Route::get('/','EbookController@index')->name('books.index');
            Route::post('/save','EbookController@save')->name('books.save');
            Route::get('/new','EbookController@new')->name('add_new_book');
            Route::get('/show/{id}','EbookController@show')->name('books.show');
            Route::get('/show/ebook/index','EbookController@getbook')->name('show.ebook.index');
            Route::get('/show/ebook/{ebookName}','EbookController@showEbook')->name('show.ebook');

            /*Route::get('/', 'QuizNewController@index');
            // get quizzes of the serie
            Route::get('/{serie}/quizzes', 'QuizNewController@serieQuizzes')->name('serie_quizzes');
            // add a quiz to a serie
            Route::match(['get', 'post'],'/add/{serie}/quiz', 'QuizNewController@add_quiz_to_serie')->name('add_quiz_to_serie');
            // display a quiz
            Route::get('/show/{quiz}', 'QuizNewController@display_quiz')->name('display_quiz');

            Route::get('/add/serie', 'QuizNewController@newSerie')->name('new_serie');
            Route::get('/create', 'QuizNewController@create');
            Route::post('/store', 'QuizController@store');
            Route::get('/{id}/edit', 'QuizController@edit')->name('adminEditQuiz');
            Route::post('/{id}/update', 'QuizController@update');
            Route::get('/{id}/delete', 'QuizController@delete');
        ...
        Route::group(['prefix' => 'quizzes'], function () {
            Route::get('/', 'QuizNewController@index');
            Route::get('/serie/{serie}/show/{currentQuizIndex}', 'QuizNewController@showSerie')->name("serie.show");


            // get quizzes of the serie
            Route::get('/{serie}/quizzes', 'QuizNewController@serieQuizzes')->name('serie_quizzes');
            // this is route for display 1 QUIZ
            Route::get('/{quiz}/show', 'QuizNewController@showQuiz')->name('quizzes.show');
                // Route::get('/{quiz}/display_quiz_for_student', 'QuizNewController@display_quiz_for_student')->name('display_quiz_for_student');
            // add a quiz to a serie
            Route::match(['get', 'post'],'/add/{serie}/quiz', 'QuizNewController@add_quiz_to_serie')->name('add_quiz_to_serie');
            // display a quiz
            Route::get('/show/{quiz}', 'QuizNewController@display_quiz')->name('display_quiz');
