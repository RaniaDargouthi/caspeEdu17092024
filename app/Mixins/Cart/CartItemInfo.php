...
class CartItemInfo
{
    public function getItemInfo($cart)
    {
        if (!empty($cart->webinar_id)) {
            ...
            return $this->getCourseInfo($cart, $book);
    ...
    private function getCourseInfo($cart, $book)
    {
        $info = [];
        ...
        $info['profileUrl'] = $book->teacher->getProfileUrl();
        $info['teacherName'] = $book->teacher->full_name;
        $info['rate'] = $book->getRate();
        $info['price'] = $book->price;
        $info['discountPrice'] = $book->getDiscount($cart->ticket) ? ($book->price - $book->getDiscount($cart->ticket)) : null;
    ...
    private function getInstallmentOrderInfo($cart, $installmentPayment)
    {
        ...
        if (!empty($installmentOrder)) {
            ...
            if (!empty($installmentOrder->webinar_id)) {
                $book = $installmentOrder->webinar;

                $info = $this->getCourseInfo($cart, $book);
