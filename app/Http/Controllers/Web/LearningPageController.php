...
class LearningPageController extends Controller
{
    ...
    public function index(Request $request, $slug)
    {
        ...
        $bookController = new WebinarController();

        $data = $bookController->course($slug, true);

        $course = $data['course'];
        $user = $data['user'];

        $installmentLimitation = $bookController->installmentContentLimitation($user, $course->id, 'webinar_id');
