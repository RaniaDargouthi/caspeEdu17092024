...
{
    public function apply(Request $request, $bookSlug)
    {
        $book = Webinar::where('slug', $bookSlug)
            ->where('status', 'active')
            ->where('subscribe', true)
            ->first();

        if (!empty($book)) {
            return $this->handleSale($book, 'webinar_id');
        }
    ...
    private function handleSale($item, $itemName = 'webinar_id')
    {
        if (auth()->check()) {
            ...
            $sale = Sale::create([
                ...
                'type' => $itemName == 'webinar_id' ? Sale::$book : Sale::$bundle,
