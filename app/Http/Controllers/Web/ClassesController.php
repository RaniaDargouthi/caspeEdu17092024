...
class ClassesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($type) and is_array($type) and in_array('bundle', $type)) {
            $booksQuery = Bundle::where('bundles.status', 'active');
            $this->tableName = 'bundles';
            $this->columnId = 'bundle_id';
        }

        $booksQuery = $this->handleFilters($request, $booksQuery);


        $sort = $request->get('sort', null);

        if (empty($sort) or $sort == 'newest') {
            $booksQuery = $booksQuery->orderBy("{$this->tableName}.created_at", 'desc');
        }

        $books = $booksQuery->with([
            'tickets'
        ])->paginate(6);
    ...
    public function handleFilters($request, $query)
    {
        ...
        if (!empty($withDiscount) and $withDiscount == 'on') {
            ...
            foreach ($tickets as $ticket) {
                if ($ticket->isValid()) {
                    $bookIdsHasDiscount[] = $ticket->{$this->columnId};
                }
            }
            ...
            $query->whereIn("{$this->tableName}.id", $bookIdsHasDiscount);
        }
        ...
        if (!empty($filterOptions) and is_array($filterOptions)) {
            $bookIdsFilterOptions = WebinarFilterOption::whereIn('filter_option_id', $filterOptions)
                ->pluck($this->columnId)
                ->toArray();

            $query->whereIn("{$this->tableName}.id", $bookIdsFilterOptions);
        }
