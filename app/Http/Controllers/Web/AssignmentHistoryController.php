...
class AssignmentHistoryController extends Controller
{
    ...
    public function storeMessage(Request $request, $assignmentId, $historyId)
    {
        ...
        if (!empty($assignment)) {
            $book = $assignment->webinar;

            if (!empty($book) and $book->checkUserHasBought($user)) {
                $studentId = $request->get('student_id');
                $assignmentHistory = $this->getAssignmentHistory($book, $assignment, $user, $studentId);
    ...
    public function setGrade(Request $request, $assignmentId, $historyId)
    {
        ...
        if (!empty($assignment)) {
            ...
            if (!empty($book) and $book->isOwner($user->id)) {
                $studentId = $request->get('student_id');
                $assignmentHistory = $this->getAssignmentHistory($book, $assignment, $user, $studentId);
    ...
    public function downloadAttach($assignmentId, $historyId, $messageId)
    {
        ...
        if (!empty($assignment)) {
            ...
            if (!empty($book) and $book->checkUserHasBought($user)) {
                $allow = ($book->creator_id == $user->id or $book->teacher_id == $user->id or $user->isAdmin());
