...
class CartController extends Controller
{
    ...
    private function handleDiscountPrice($discount, $carts, $subTotal)
    {
        ...
        if ($discount->source == Discount::$discountSourceCourse) {
            ...
            $bookOtherDiscounts = 0;
            $discountWebinarsIds = $discount->discountCourses()->pluck('course_id')->toArray();

            foreach ($carts as $cart) {
                $book = $cart->webinar;
                if (!empty($book) and in_array($book->id, $discountWebinarsIds)) {
                    $totalWebinarsAmount += $book->price;
                    //$bookOtherDiscounts += $book->getDiscount($cart->ticket, $user);
                }
            }
            ...
            if ($discount->discount_type == Discount::$discountTypeFixedAmount) {
                ...
                /*if (!empty($bookOtherDiscounts)) {
                    $totalDiscount = $totalDiscount - (int)$bookOtherDiscounts;
                }*/
            } else {
            ...
            foreach ($carts as $cart) {
                ...
                if (!empty($book) and in_array($book->category_id, $categoriesIds)) {
                    $totalCategoriesAmount += $book->price;
                    //$categoriesOtherDiscounts += $book->getDiscount($cart->ticket, $user);
                }
            }
            ...
            foreach ($carts as $cart) {
                ...
                if (!empty($book)) {
                    $totalCartAmount += $book->price;
                    //$totalCartOtherDiscounts += $book->getDiscount($cart->ticket, $user);
                }
