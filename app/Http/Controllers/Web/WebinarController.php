...
class WebinarController extends Controller
{
    ...
    public function course($slug, $justReturnData = false)
    {
        ...
        if (!empty($course->sessions)) {
            $bookContentCount += $course->sessions->count();
        }
        if (!empty($course->files)) {
            $bookContentCount += $course->files->count();
        }
        if (!empty($course->textLessons)) {
            $bookContentCount += $course->textLessons->count();
        }
        if (!empty($course->quizzes)) {
            $bookContentCount += $course->quizzes->count();
        }
        if (!empty($course->assignments)) {
            $bookContentCount += $course->assignments->count();
        }
        ...
        $data = [
            ...
            'hasBought' => $hasBought,
            'user' => $user,
            'webinarContentCount' => $bookContentCount,
    ...
    public function downloadFile($slug, $file_id)
    {
        ...
        if (!empty($book) and $this->checkCanAccessToPrivateCourse($book)) {
            $file = File::where('webinar_id', $book->id)
                ->where('id', $file_id)
                ->first();
            ...
            if (!empty($file) and $file->downloadable) {
                ...
                if ($file->accessibility == 'paid') {
                    $canAccess = $book->checkUserHasBought();
                }
    ...
    public function getFilePath(Request $request)
    {
        ...
        if (!empty($file)) {
            $book = Webinar::where('id', $file->webinar_id)
                ->where('status', 'active')
                ->with([
            ...
            if (!empty($book)) {
                ...
                if ($canAccess) {
                    ...
                    if ($file->storage == 'upload') {
                        $path = url("/course/$book->slug/file/$file->id/play");
                    } elseif ($file->storage == 'upload_archive') {
                        $path = url("/course/$book->slug/file/$file->id/showHtml");
                    }
    ...
    public function directPayment(Request $request)
    {
        ...
        if (!empty($user) and !empty(getFeaturesSettings('direct_classes_payment_button_status'))) {
            ...
            if (!empty($book)) {
                $checkCourseForSale = checkCourseForSale($book, $user);
