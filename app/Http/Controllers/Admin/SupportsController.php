...
class SupportsController extends Controller
{
    ...
    private function handleFilters($request, $query)
    {
        ...
        $student_ids = $request->get('student_ids');
        $book_ids = $request->get('webinar_ids');
        $bookTitle = $request->get('webinar_title');
        ...
        if (!empty($bookTitle)) {
            $book_ids = Webinar::where('status', 'active')
                ->whereTranslationLike('title', "%$bookTitle%")
                ->pluck('id')
                ->toArray();
        }
        ...
        if (!empty($teacher_ids)) {
            ...
            $bookIds = Webinar::where('status', 'active')
            ...
            $query->where(function ($query) use ($bookIds, $teacher_ids) {
                $query->whereIn('webinar_id', $bookIds)
