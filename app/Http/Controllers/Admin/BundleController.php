            ...
            foreach ($students as $key => $student) {
                ...
                foreach ($bundleWebinars as $bundleWebinar) {
                    if (!empty($bundleWebinar->webinar)) {
                        $bookCount += 1;
                        $learnings += $bookStatisticController->getCourseProgressForStudent($bundleWebinar->webinar, $student->id);
                    }
                }

                $learnings = ($learnings > 0 and $bookCount > 0) ? round($learnings / $bookCount, 2) : 0;
