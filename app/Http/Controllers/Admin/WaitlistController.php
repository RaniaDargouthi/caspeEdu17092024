...
class WaitlistController extends Controller
{
    ...
    public function viewList(Request $request, $bookId, $justReturnQuery = false)
    {
        $this->authorize('admin_waitlists_users');

        $book = Webinar::query()->findOrFail($bookId);

        $query = Waitlist::query()->where('webinar_id', $bookId);
        ...
        $data = [
            'pageTitle' => trans('update.waitlists') . ' - ' . $book->title,
            'waitlists' => $waitlists,
            'waitlistId' => $bookId,
            'webinarTitle' => $book->title,
        ];

        return view('admin.waitlists.users_list', $data);
    }

    public function clearList(Request $request, $bookId)
    {
        $this->authorize('admin_waitlists_clear_list');

        Waitlist::query()->where('webinar_id', $bookId)
            ->delete();
    ...
    public function disableWaitlist(Request $request, $bookId)
    {
    ...
    public function exportUsersList(Request $request, $bookId)
    {
        $this->authorize('admin_waitlists_exports');

        $waitlists = $this->viewList($request, $bookId, true)
            ->with(['user'])
            ->orderBy('created_at', 'desc')
            ->get();
