...
class WebinarController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->get('status', null) == 'active_finished') {
            foreach ($books as $key => $book) {
                if ($book->last_date > time()) { // is in progress
                    unset($books[$key]);
                }
            }
        }
        ...
        foreach ($books as $book) {
            $giftsIds = Gift::query()->where('webinar_id', $book->id)
                ->where('status', 'active')
            ...
            $sales = Sale::query()
                ->where(function ($query) use ($book, $giftsIds) {
                    $query->where('webinar_id', $book->id);
        ...
        if (!empty($sort)) {
            switch ($sort) {
                case 'has_discount':
                    ...
                    $bookIdsHasDiscount = array_merge($specialOffersWebinarIds, $bookIdsHasDiscount);

                    $query->whereIn('id', $bookIdsHasDiscount)
    ...
    {
        $count = 0;
        $books = Webinar::where('type', 'webinar')
            ->where('status', 'active')
            ->where('start_date', '<=', time())
            ->whereHas('sessions')
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($data['type'] != Webinar::$book) {
            $data['start_date'] = null;
        }

        if (!empty($data['start_date']) and $data['type'] == Webinar::$book) {
        ...
        if (!empty($filters) and is_array($filters)) {
            WebinarFilterOption::where('webinar_id', $book->id)->delete();
        ...
        if (!empty($request->get('tags'))) {
            ...
            Tag::where('webinar_id', $book->id)->delete();
        ...
        if (!empty($request->get('partner_instructor')) and !empty($request->get('partners'))) {
            WebinarPartnerTeacher::where('webinar_id', $book->id)->delete();
        ...
        return redirect(getAdminPanelUrl() . '/webinars/' . $book->id . '/edit?locale=' . $data['locale']);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        storeContentLocale($locale, $book->getTable(), $book->id);
        ...
        $teacherQuizzes = Quiz::where('webinar_id', null)
            ->where('creator_id', $book->teacher_id)
            ->get();

        $tags = $book->tags->pluck('title')->toArray();

        $data = [
            'pageTitle' => trans('admin/main.edit') . ' | ' . $book->title,
            'categories' => $categories,
            'webinar' => $book,
            'webinarCategoryFilters' => !empty($book->category) ? $book->category->filters : null,
            'webinarFilterOptions' => $book->filterOptions->pluck('filter_option_id')->toArray(),
            'tickets' => $book->tickets,
            'chapters' => $book->chapters,
            'sessions' => $book->sessions,
            'files' => $book->files,
            'textLessons' => $book->textLessons,
            'faqs' => $book->faqs,
            'assignments' => $book->assignments,
            'teacherQuizzes' => $teacherQuizzes,
            'prerequisites' => $book->prerequisites,
            'webinarQuizzes' => $book->quizzes,
            'webinarPartnerTeacher' => $book->webinarPartnerTeacher,
            'webinarTags' => $tags,
    ...
    public function update(Request $request, $id)
    {
        ...
        $rules = [
            'type' => 'required|in:webinar,course,text_lesson',
            'title' => 'required|max:255',
            'slug' => 'max:255|unique:webinars,slug,' . $book->id,
            'thumbnail' => 'required',
            'image_cover' => 'required',
            'description' => 'required',
            'teacher_id' => 'required|exists:users,id',
        ...
        if (!empty($data['teacher_id'])) {
            ...
            $creator = !empty($data['organ_id']) ? User::find($data['organ_id']) : $book->creator;
        ...
        if (!empty($data['start_date']) and $book->type == 'webinar') {
            if (empty($data['timezone']) or !getFeaturesSettings('timezone_in_create_webinar')) {
                $data['timezone'] = getTimezone();
            }
        ...
        $changedCreator = ($book->creator_id != $newCreatorId);
        ...
        if ($publish) {
            sendNotification('course_approve', ['[c.title]' => $book->title], $book->teacher_id);
            ...
            sendNotification('course_reject', ['[c.title]' => $book->title], $book->teacher_id);
        }

        if ($changedCreator) {
            $this->webinarChangedCreator($book);
        }
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $book = Webinar::query()->findOrFail($id);
    ...
    public function exportExcel(Request $request)
    {
        ...
        $bookExport = new WebinarsExport($books);

        return Excel::download($bookExport, 'webinars.xlsx');
    }
    ...
    public function studentsLists(Request $request, $id)
    {
        ...
        if (!empty($book)) {
            ...
            $query = User::join('sales', 'sales.buyer_id', 'users.id')
                ->leftJoin('webinar_reviews', function ($query) use ($book) {
                    $query->on('webinar_reviews.creator_id', 'users.id')
                        ->where('webinar_reviews.webinar_id', $book->id);
                })
                ...
                ->where(function ($query) use ($book, $giftsIds, $installmentSalesIds) {
                    $query->where('sales.webinar_id', $book->id);
            ...
            $students = $this->studentsListsFilters($book, $query, $request)
            ...
            if (!empty($book->access_days)) {
                $accessTimestamp = $book->access_days * 24 * 60 * 60;
            ...
            $bookStatisticController = new WebinarStatisticController();
            ...
            foreach ($allStudentsIds as $studentsId) {
                $learningPercents[$studentsId] = $bookStatisticController->getCourseProgressForStudent($book, $studentsId);
            }
                ...
                if (!empty($student->gift_id)) {
                    ...
                    if (!empty($gift)) {
                        ...
                        if (!empty($receipt)) {
                            ...
                            $receipt->learning = $bookStatisticController->getCourseProgressForStudent($book, $receipt->id);
    ...
    private function studentsListsFilters($book, $query, $request)
    {
        ...
        if (!empty($status)) {
            if ($status == 'expire' and !empty($book->access_days)) {
                $accessTimestamp = $book->access_days * 24 * 60 * 60;
    ...
    public function sendNotificationToStudents(Request $request, $id)
    {
        ...
        if (!empty($book)) {
            ...
            $toastData = [
                'title' => trans('public.request_success'),
                'msg' => trans('update.the_notification_was_successfully_sent_to_n_students', ['count' => count($book->sales)]),
                'status' => 'success'
            ];

            return redirect(getAdminPanelUrl("/webinars/{$book->id}/students"))->with(['toast' => $toastData]);
        }
    ...
    public function getContentItemByLocale(Request $request, $id)
    {
        ...
        $book = Webinar::where('id', $id)->first();
        ...
        if (!empty($book)) {
            ...
            if (!empty($book->$relation)) {
                $item = $book->$relation->where('id', $itemId)->first();
