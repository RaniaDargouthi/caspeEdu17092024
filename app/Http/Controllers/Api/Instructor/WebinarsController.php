...
class WebinarsController extends Controller
{
    ...
    private function makeMyClassAndInvitationsData($query, $user, $request)
    {
        ...
        foreach ($bookSales as $bookSale) {
            if ($bookSale->webinar->type == 'webinar') {
    ...
    public function storeAll(Request $request)
    {
        ...
        $bookRulesRequired = (!empty($data['rules']) && $data['rules'] == 1);

        $data['status'] = ($isDraft or !$bookRulesRequired) ? Webinar::$isDraft : Webinar::$pending;
        $data['private'] = (!empty($data['private']) and $data['private'] == 1);

        if ($book_type == 'webinar') {
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($currentStep == 1) {
            ...
            $book->slug = null; // regenerate slug in model
        }
        ...
        if ($getNextStep) {
            ...
            $url = '/panel/webinars/' . $book->id . '/step/' . (($nextStep <= 8) ? $nextStep : 8);
        }
        ...
        if (!$getNextStep and !$isDraft and !$bookRulesRequired) {
    ...
    public function search(Request $request)
    {
        ...
        if (!empty($term)) {
            $books = Webinar::select('id', 'title', 'teacher_id')
    ...
    {
        $user = auth()->user();
        $bookIds = $user->getPurchasedCoursesIds();

        $query = Webinar::whereIn('id', $bookIds);
        ...
        foreach ($allWebinars as $book) {
            if (!empty($book->start_date) and $book->start_date > $time) {
                $upComing += 1;
            }
        }
        ...
        foreach ($books as $book) {
            ...
            if (!empty($sale)) {
                $book->purchast_date = $sale->created_at;
            }
        }
