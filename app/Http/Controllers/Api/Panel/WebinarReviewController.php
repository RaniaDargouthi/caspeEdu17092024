...
class WebinarReviewController extends Controller
{
    public function list(Request $request)
    {
        $user = apiAuth();
        //  $user=User::find(1) ;
        $bookReview = WebinarReview::where('creator_id', $user->id)->get()->map(function ($review) {
            return $review->details;
        });
        return apiResponse2(1, 'retrieved', trans('api.public.retrieved'), $bookReview);
    }
    ...
    public function store(Request $request)
    {
        ...
        $user = apiAuth();
        $book = Webinar::where('id', $request->input('webinar_id'))
            ->where('status', 'active')
            ->first();
        ...
        $bookReview = WebinarReview::where('creator_id', $user->id)
        ...
        sendNotification('new_rating', $notifyOptions, $book->teacher_id);
