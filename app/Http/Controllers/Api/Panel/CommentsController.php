...
class CommentsController extends Controller
{
    ...
    public function myComments(Request $request)
    {
        ...
        $book_query = clone $query;
        $book_comments = $book_query->whereNotNull('webinar_id')
            ->handleFilters()->orderBy('created_at', 'desc')
        ...
        return ['blogs' => $blog_comments, 'webinar' => $book_comments];
    ...
    private function handleFilter($query, $request)
    {
        ...
        if (!empty($book)) {
            $booksIds = Webinar::where('title', 'like', "%$book%")->pluck('id')->toArray();

            $query->whereIn('webinar_id', $booksIds);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($item_name == 'webinar_id') {
            $book = Webinar::FindOrFail($item_id);
            $notifyOptions = [
                '[c.title]' => $book->title,
