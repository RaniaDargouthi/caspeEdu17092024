...
class FileController extends Controller
{
        ...
        if (!empty($book) and $book->canAccess($user)) {
            ...
            if ($data['storage'] == 'upload_archive') {
                ...
                if ($data['storage'] == 's3') {
                    ...
                    $result = $this->uploadFileToBunny($book, $data['s3_file']);
                }
    ...
    private function uploadFileToBunny($book, $file)
    {
        $result = [
            'path' => null,
            'status' => true
        ...
        try {
            ...
            $collectionId = $bunnyVideoStream->createCollection("course {$book->id}", true);
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if (!empty($file)) {
            $book = Webinar::query()->find($file->webinar_id);

            if ($file->creator_id == $user->id or (!empty($book) and $book->canAccess($user))) {
