...
class QuizController extends Controller
{
    ...
    {
        $user = auth()->user();
        $books = Webinar::where(function ($query) use ($user) {
    ...
    public function edit(Request $request, $id)
    {
        ...
        $quiz = Quiz::where('id', $id)
            ->where('creator_id', $user->id)
            ->where(function ($query) use ($user, $bookIds) {
                $query->where('creator_id', $user->id);
                $query->orWhereIn('webinar_id', $bookIds);
            })
            ->with([
    ...
    public function destroy(Request $request, $id)
    {
        ...
        if (!empty($quiz)) {
            ...
            if (!empty($quiz->webinar_id)) {
                $book = Webinar::query()->find($quiz->webinar_id);
            }

            if ($quiz->creator_id == $user->id or (!empty($book) and $book->canAccess($user))) {
                if ($quiz->delete()) {
    ...
    public function start(Request $request, $id)
    {
        ...
        if ($quiz) {
            ...
            if (!empty($quiz->webinar_id)) {
                ...
                $checkUserHasBought = $book->checkUserHasBought($user);
    ...
    public function opens(Request $request)
    {
        ...
        $query = Quiz::whereIn('webinar_id', $bookIds)
            ->where('status', 'active')
