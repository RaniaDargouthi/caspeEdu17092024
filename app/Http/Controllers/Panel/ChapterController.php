...
class ChapterController extends Controller
{
    public function getChapter(Request $request, $id)
    {
        ...
        if (!empty($chapter)) {
            $book = Webinar::query()->find($chapter->webinar_id);

            if ($chapter->user_id == $user->id or (!empty($book) and $book->canAccess($user))) {
    ...
    public function getAllByWebinarId($book_id)
    {
        $user = auth()->user();
        ...
        if (!empty($book) and $book->canAccess($user)) {
            ...
            $chapters = $book->chapters->where('status', WebinarChapter::$chapterActive);
