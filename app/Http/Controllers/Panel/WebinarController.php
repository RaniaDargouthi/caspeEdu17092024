...
class WebinarController extends Controller
{
    ...
    {
        $bookHours = deepClone($query)->sum('duration');
        ...
        $bookSales = Sale::where('seller_id', $user->id)
            ->where('type', 'webinar')
        ...
        foreach ($bookSales as $bookSale) {
            if (!empty($bookSale->webinar) and $bookSale->webinar->type == 'webinar') {
                $bookSalesAmount += $bookSale->amount;
            } else {
                $courseSalesAmount += $bookSale->amount;
            }
        }
        ...
        return [
            'webinars' => $books,
            'webinarsCount' => $booksCount,
            'webinarSalesAmount' => $bookSalesAmount,
            'courseSalesAmount' => $courseSalesAmount,
            'webinarHours' => $bookHours,
        ];
    }
    ...
    public function store(Request $request)
    {
        ...
        $url = '/panel/webinars';
        if ($data['get_next'] == 1) {
            $url = '/panel/webinars/' . $book->id . '/step/2';
        }
    ...
    public function edit(Request $request, $id, $step = 1)
    {
        ...
        $data['pageTitle'] = trans('public.edit') . ' ' . $book->title;

        $definedLanguage = [];
        if ($book->translations) {
            $definedLanguage = $book->translations->pluck('locale')->toArray();
        }

        $data['definedLanguage'] = $definedLanguage;

        if ($step == 2) {
            $data['webinarTags'] = $book->tags->pluck('title')->toArray();

            $bookCategoryFilters = !empty($book->category) ? $book->category->filters : [];

            if (empty($book->category) and !empty($request->old('category_id'))) {
                $category = Category::where('id', $request->old('category_id'))->first();

                if (!empty($category)) {
                    $bookCategoryFilters = $category->filters;
                }
            }

            $data['webinarCategoryFilters'] = $bookCategoryFilters;
        }

        if ($step == 3) {
            $data['sumTicketsCapacities'] = $book->tickets->sum('capacity');
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$directPublicationOfCourses and (($currentStep == 8 and !$getNextStep and !$isDraft) or (!$getNextStep and !$isDraft))) {
            $bookRulesRequired = empty($data['rules']);
        }

        $this->validate($request, $rules);

        $status = ($isDraft or $bookRulesRequired) ? Webinar::$isDraft : Webinar::$pending;
        ...
        if ($currentStep == 2) {
            ...
            // Check Capacity
            $userPackage = new UserPackage($book->creator);
        ...
        if (empty($data['teacher_id']) and $user->isOrganization() and $book->creator_id == $user->id) {
        ...
        if ($getNextStep) {
            ...
            $url = '/panel/webinars/' . $book->id . '/step/' . (($nextStep <= $stepCount) ? $nextStep : $stepCount);
        }

        if ($bookRulesRequired) {
            $url = '/panel/webinars/' . $book->id . '/step/8';

            return redirect($url)->withErrors(['rules' => trans('validation.required', ['attribute' => 'rules'])]);
        }

        if ($status != Webinar::$active and !$getNextStep and !$isDraft and !$bookRulesRequired) {
            sendNotification('course_created', ['[c.title]' => $book->title], $user->id);
    ...
    public function duplicate($id)
    {
        ...
        if (!empty($book)) {
            ...
            $title = $book->title . ' ' . trans('public.copy');
            ...
            foreach ($book->translatedAttributes as $attribute) {
                unset($new[$attribute]);
            }
            ...
            WebinarTranslation::updateOrCreate([
                ...
                'locale' => mb_strtolower($book->locale),
            ], [
                'title' => $title,
    ...
    public function search(Request $request)
    {
        ...
        $bookId = $request->get('webinar_id', null);
        $option = $request->get('option', null);

        if (!empty($term)) {
            $books = Webinar::select('id', 'teacher_id')
            ...
            foreach ($books as $book) {
                $book->title .= ' - ' . $book->teacher->full_name;
            }
    ...
    public function invoice($bookId, $saleId)
    {
        $user = auth()->user();
        ...
        $sale = Sale::query()
            ->where('id', $saleId)
            ->where(function ($query) use ($bookId, $user, $giftIds) {
                $query->where(function ($query) use ($bookId, $user) {
    ...
    public function purchases(Request $request)
    {
        ...
        $booksHours = deepClone($query)->join('webinars', 'webinars.id', 'sales.webinar_id')
        ...
        $hours = $booksHours + $bundlesHours + $giftDurations;
    ...
    public function getNextSessionInfo($id)
    {
        ...
        if (!empty($book)) {
            $session = Session::where('webinar_id', $book->id)
                ->where('date', '>=', time())
                ->orderBy('date', 'asc')
        ...
        if (!empty($itemIds) and is_array($itemIds) and count($itemIds)) {
            switch ($tableName) {
                ...
                case 'text_lessons':
                    ...
                    break;
                case 'prerequisites':
                    $bookIds = $user->webinars()->pluck('id')->toArray();

                    foreach ($itemIds as $order => $id) {
                        Prerequisite::where('id', $id)
                            ->whereIn('webinar_id', $bookIds)
                            ->update(['order' => ($order + 1)]);
                    }
                    break;
                case 'faqs':
