...
class SupportsController extends Controller
{
    public function index(Request $request, $id = null)
    {
        ...
        $bookIds = array_merge($purchasedWebinarsIds, $userWebinarsIds);
        ...
        $teacherIds = $books->pluck('teacher_id')->toArray();
    ...
    private function filters($query, $request, $userWebinarsIds = [])
    {
        ...
        if (!empty($book_id) and $book_id != 'all') {
    ...
    public function create()
    {
        ...
        $books = Webinar::select('id', 'creator_id')
            ->whereIn('id', $bookIds)
            ->where('support', true)
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($data['webinar_id'])) {
            ...
            sendNotification('support_message', $notifyOptions, $book->teacher_id);
        }
    ...
    public function storeConversations(Request $request, $id)
    {
        ...
        if (!empty($support->webinar_id)) {
            $book = Webinar::findOrFail($support->webinar_id);

            $notifyOptions = [
                '[c.title]' => $book->title,
            ];
            sendNotification('support_message_replied', $notifyOptions, ($support->user_id == $user->id) ? $book->teacher_id : $user->id);
        }
