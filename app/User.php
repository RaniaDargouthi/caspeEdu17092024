...
class User extends Authenticatable
{
    ...
    public function rates()
    {
        ...
        if (!empty($books)) {
            ...
            foreach ($books as $book) {
                ...
                if (!empty($bookRate) and $bookRate > 0) {
                    $count += 1;
                    $rates += $bookRate;
                }
            }
    ...
    {
        $bookIds = Webinar::where('creator_id', $this->id)->pluck('id')->toArray();

        return Sale::whereIn('webinar_id', $bookIds)->sum('amount');
    }
    ...
    public function getPurchasedCoursesIds()
    {
        ...
        if (!empty($bundleIds)) {
            ...
            $bookIds = array_merge($bookIds, $bundleWebinarIds);
        }

        return array_unique($bookIds);
    }
