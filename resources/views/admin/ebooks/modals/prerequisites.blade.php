...
<div class="d-none" id="webinarPrerequisitesModal">
    ...
    <div class="js-prerequisites-form" data-action="{{ getAdminPanelUrl() }}/prerequisites/store" >
        <input type="hidden" name="webinar_id" value="{{  !empty($book) ? $book->id :''  }}">

        <div class="form-group mt-15">
            <label class="input-label d-block">{{ trans('public.select_prerequisites') }}</label>
            <select id="prerequisitesSelect" name="prerequisite_id" class="js-ajax-prerequisite_id form-control prerequisites-select" data-webinar-id="{{  !empty($book) ? $book->id : '' }}" data-placeholder="{{ trans('public.search_prerequisites') }}">
