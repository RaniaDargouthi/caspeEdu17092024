...
@section('content')
    <section class="">
        ...
        @endif
        {{-- class= dashboard-banner-container --}}
        <style>
            .background-div {
                background-image: url('{{ getPageBackgroundSettings('dashboard') }}');
        ...
        <div class="bg-white position-relative px-15 px-ld-35 py-10 panel-shadow rounded-sm background-div">
            ...
            <h2 class="font-30 text-primary line-height-1">
                <span class="d-block">{{ trans('admin/main.welcome') }} {{ $authUser->full_name }},</span>
                <span
            ...
            <div class="col-12 col-lg-3 mt-35">
                <a href="@if ($authUser->isUser()) /panel/webinars/purchases @else /panel/meetings/requests @endif"
                    ...
                    <div class="d-flex flex-column ml-15">
                        <span
                            class="font-30 text-secondary">{{ !empty($pendingAppointments) ? $pendingAppointments : (!empty($booksCount) ? $booksCount : 0) }}</span>
                        <span
