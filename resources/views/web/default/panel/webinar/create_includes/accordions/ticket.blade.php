    ...
    <div id="collapseTicket{{ !empty($ticket) ? $ticket->id :'record' }}" aria-labelledby="ticket_{{ !empty($ticket) ? $ticket->id :'record' }}" class=" collapse @if(empty($ticket)) show @endif" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form ticket-form" data-action="/panel/tickets/{{ !empty($ticket) ? $ticket->id . '/update' : 'store' }}">
                <input type="hidden" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][webinar_id]" value="{{ !empty($book) ? $book->id :'' }}">
                        ...
                        <div class="col-12 col-lg-6">
                            <div class="form-group">
                                ...
                                <select name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][locale]"
                                        ...
                                        data-webinar-id="{{ !empty($book) ? $book->id : '' }}"
                ...
                <div class="row">
                    <div class="col-12 col-lg-4">
                        ...
                        <div class="form-group">
                            ...
                            @if(empty($ticket) and !empty($book->capacity) and !empty($sumTicketsCapacities))
                                <span class="test-gray font-12 d-block">{{ trans('panel.remaining') }}: <span class="js-ticket-remaining-capacity">{{ $book->capacity - $sumTicketsCapacities }}</span></span>
                            @endif

                            <input type="text" name="ajax[{{ !empty($ticket) ? $ticket->id : 'new' }}][capacity]" class="js-ajax-capacity form-control mt-10" value="{{ !empty($ticket) ? $ticket->capacity :'' }}" placeholder="{{ $book->isWebinar() ? trans('webinars.empty_means_webinar_capacity') : trans('forms.empty_means_unlimited') }}"/>
                            <div class="invalid-feedback"></div>
                        </div>
                    </div>
                </div>
