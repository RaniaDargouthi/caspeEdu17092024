...
@section('content')
    <div class="">
        ...
        <form method="post" action="/panel/webinars/{{ !empty($book) ? $book->id .'/update' : 'store' }}" id="webinarForm" class="webinar-form">
        ...
        <div class="create-webinar-footer d-flex flex-column flex-md-row align-items-center justify-content-between mt-20 pt-15 border-top">
            <div class="d-flex align-items-center">
                ...
                @if(!empty($book))
                    <a href="/panel/webinars/{{ $book->id }}/step/{{ ($currentStep - 1) }}" class="btn btn-sm btn-primary {{ $currentStep < 2 ? 'disabled' : '' }}">{{ trans('webinars.previous') }}</a>
                @else
            ...
            <div class="mt-20 mt-md-0">
                ...
                @if(!empty($book) and $book->creator_id == $authUser->id)
                    <a href="/panel/webinars/{{ $book->id }}/delete?redirect_to=/panel/webinars" class="delete-action webinar-actions btn btn-sm btn-danger mt-20 mt-md-0">{{ trans('public.delete') }}</a>
                @endif
            </div>
        </div>
    </div>
